...
const cardData: CardInfo[] = [
  { category: "RFQ", iconComponent: CommercialIcon },
];

const CommercialCards: React.FC<CardsProps> = ({ onCardClick }) => {
  ...
  return (
    <div className="cardMenu">
      {cardData.map((card, index) => (
        ...
        >
          <card.iconComponent className="cardIcon" size="large" />
          <h1>
            {card.category} <br /> Comparison
          </h1>
        </div>
      ))}
    </div>
